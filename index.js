...
class chat_record_class extends PIXI.Container {
	...
	constructor() {
		...
		this.msg = new PIXI.BitmapText('Имя Фамил', {fontName: 'mfont32',fontSize: 19,lineSpacing:32,align: 'left'});
...
rsp={
	...
	bcg_down(e){
		...
		if (py<290) return
		this.my_opt=2
		if (px<355) this.my_opt=1
		if (px>435) this.my_opt=3
...
lobby={
	...
	async show_instr(part){
		...
		if (part===1){
			...
			await anim3.add(objects.hand_instr,{x:[450, 68,'easeOutCubic'],y:[800, 84,'easeOutCubic']}, true, 0.5)
			await new Promise(resolve => setTimeout(resolve, 250))
			for (let i=0;i<3;i++){
		...
		if (part===2){
			this.first_run=0
			objects.hand_instr.texture=assets.hand0		
			await anim3.add(objects.hand_instr,{x:[68, 622,'easeOutCubic'],y:[84, 181,'easeOutCubic']}, true, 0.5)
			await new Promise(resolve => setTimeout(resolve, 250))
			for (let i=0;i<3;i++){
				objects.hand_instr.texture=assets.hand1
				await new Promise(resolve => setTimeout(resolve, 250))
...
main_loader={
	...
	async load1(){
		...
		git_src='https://monopoly.hb.ru-msk.vkcloud-storage.ru/'
		git_src=''
